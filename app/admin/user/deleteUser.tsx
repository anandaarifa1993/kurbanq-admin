...
const DeleteUser = ({ selectedUser }: { selectedUser: IUser }) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    try {
      const url = `${BASE_API_URL}/user/delete/${selectedUser.idUser}`;
        ...
        <form onSubmit={handleSubmit}>
          <div className="sticky top-0 bg-white px-5 pt-5 pb-3 shadow">
            <div className="flex justify-between items-center">
              <div>
                <strong className="text-2xl">Hapus User</strong>
                <small className="block text-slate-400 text-sm">
                  Data user yang terhubung dengan transaksi tidak dapat dihapus.
                </small>
              </div>
              <button
                type="button"
